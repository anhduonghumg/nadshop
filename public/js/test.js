...
function dateFormatYMDToDMYYTask(string_date) {
    ...
    if (y == date_now.getUTCFullYear()) {
        return d + (m <= 9 ? ' thĂ¡ng ' + m : ' Th' + m);
    }
    return d + (m <= 9 ? ' thĂ¡ng ' + m : " Th" + m) + " " + y;
}
...
function setDataTable(id, numberRecord) {
    $('#' + id).DataTable({
        "destroy": true,
        // "searching": false,         // áº¨n Ă´ search
        "ordering": true,           // áº¨n sáº¯p xáº¿p cĂ¡c cá»™t
        // "lengthChange": true,      // áº¨n pháº§n chá»‰nh sá»‘ lÆ°á»£ng hiá»ƒn thá»‹
        "info": false,              // áº¨n pháº§n info (Ex: Showing 1 to 20 of 100 entries)
        "pageLength": numberRecord,           // Máº·c Ä‘á»‹nh hiá»ƒn thá»‹ 20
        "columnDefs": [
        ...
        language: {
            search: "_INPUT_",
            searchPlaceholder: "TĂ¬m kiáº¿m...",
            lengthMenu: " Hiá»ƒn thá»‹ _MENU_ báº£n ghi ", //lengthMenu:" _MENU_ ",
            paginate: {
                previous: 'â€¹',
                next: 'â€º'
            },
...
function moreText(maxLength) {
    $(".show-read-more").each(function () {
        ...
        if ($.trim(myStr).length > maxLength) {
            ...
            $(this).append(' <a href="javascript:void(0);" class="read-more">thĂªm...</a>');
